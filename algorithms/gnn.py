...
for iter in range(training_iterations):
    ...
    print('\n> The Discriminator value V_D(=E[ ln D(x)]) : ', V_D)
    ...
    print('\n> The Generator value V_D(=E[ ln (1-D(x))]) : ', V_G)

    V_DG = V_D + V_G

    print('\n> The Computed cost V_DG : ', V_DG)
    ...
    for i in range(n):
        delta_theta[i] = 0
        for j in range(m):
            print('> Alpha {} , Beta {}'.format(i + 1, j + 1), ' : ', alpha_beta[j][i])
