...
def basic_metrics(y_true, y_pred, class_names, avg, normalize=False):
    ...
    print("\t\t", "{:.4f}".format(precision_score(y_true, y_pred, average=avg)), "\t",
          "{:.4f}".format(recall_score(y_true, y_pred, average=avg)), "\t",
          "{:.4f}".format(f1_score(y_true, y_pred, average=avg)))
...
def main():
    ...
    class_names = np.array(["Three", "Two", "One"])  # Name of classes in descending order
    y_true = [1, 1, 1, 1, 2, 2, 3, 3, 1]
    y_pred = [1, 2, 1, 3, 1, 2, 3, 3, 1]
